...
func ImportModuleLevelObject(ctx Context, name string, globals, locals StringDict, fromlist Tuple, level int) (Object, error) {
	...
	// Fall back to loading .py files from sys.path for now so the upstream
	// parser/compiler/VM tests can run unchanged in this fork.
	...
	if err != nil {
		if IsException(FileNotFoundError, err) {
			return nil, ExceptionNewf(ImportError, "No module named %q", name)
		}
		return nil, err
	}
