...
func (vm *Vm) Call(argc int32, starArgs py.Object, starKwargs py.Object) error {
	...
	if extFn, ok := fn.(*py.ExtFunction); ok {
		vm.extCall = newFrameExitExtCall(extFn, args, kwargs, vm.frame)
		vm.why = whyExternalCall
		return nil
	}
	...
	if err != nil {
		if extCall, ok := err.(*FrameExitExtCall); ok {
			extCall.AppendPausedFrame(vm.frame)
			vm.extCall = extCall
			vm.why = whyExternalCall
			return nil
		}
...
//
// Returns an Object and an error. The error will be a py.ExceptionInfo when
// execution terminates with an uncaught exception.
func RunFrame(frame *py.Frame) (res py.Object, err error) {
	return runFrame(frame, nil)
}

// RunFrameWithException resumes the frame and injects an exception before any
// further opcode executes.
func RunFrameWithException(frame *py.Frame, exc py.ExceptionInfo) (res py.Object, err error) {
	return runFrame(frame, &exc)
}

func runFrame(frame *py.Frame, injected *py.ExceptionInfo) (res py.Object, err error) {
	var vm = Vm{
		frame:   frame,
	...
	for {
		...
		if vm.why == whyExternalCall {
			goto fast_ext_call
		}

		// Something exceptional has happened - unwind the block stack and find out what.
...
fast_ext_call:
	if vm.extCall == nil {
		return nil, py.ExceptionNewf(py.SystemError, "vm paused without external call details")
	}
	return nil, vm.extCall
